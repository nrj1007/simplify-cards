...
const runComparison = (query: string) => {
  console.log(`======================================`);
  ...
  targets.forEach(item => {
    ...
    console.log(`Rank in results: ${scores.findIndex(s => s.card.id === item.card.id) + 1} / ${scores.length}`);
...
runComparison("");
runComparison("infinia vs burgundy magnus");
runComparison("top cards");
runComparison("best super premium card");
