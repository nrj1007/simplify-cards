...
const issues = [];
const warnings = [];
const cardsDir = node_path_1.default.join(root, "data", "cards");
const cardFiles = node_fs_1.default
    .readdirSync(cardsDir)
    .filter((file) => file.endsWith(".json"))
    .sort();
const cards = cardFiles.flatMap((file) => {
    const filePath = node_path_1.default.join(cardsDir, file);
    const raw = node_fs_1.default.readFileSync(filePath, "utf8");
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) {
        addIssue(`${node_path_1.default.join("data", "cards", file)} must contain an array`);
        return [];
    }
...
function addIssue(message, cardId, field) {
    issues.push({ cardId, field, message });
}
function addWarning(message, cardId, field) {
...
function validateMoney(card, cardId, field, nullable = false) {
    const value = card[field];
    if (nullable && value === null)
        return;
    if (!isMoneyNumber(value))
        addIssue("must be a non-negative number" + (nullable ? " or null" : ""), cardId, field);
}
function validateLounge(value, cardId, field) {
    ...
    if (!validNumber && !validString)
        addIssue('must be a non-negative integer or "unlimited"', cardId, field);
}
function validateDate(value, cardId, field) {
    if (!isNonEmptyString(value)) {
    ...
    if (Number.isNaN(date.getTime()))
        addIssue("must be a valid date", cardId, field);
}
if (cardFiles.length === 0) {
    ...
    cards.forEach(({ card, file }, index) => {
        if (!isObject(card)) {
            addIssue(`${node_path_1.default.join("data", "cards", file)} card at index ${index} must be an object`);
            return;
        }
        const cardId = isNonEmptyString(card.id) ? card.id : `index:${index}`;
        if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(cardId))
            addIssue("must be kebab-case lowercase id", cardId, "id");
        if (ids.has(cardId))
            addIssue("duplicate card id", cardId, "id");
        ids.add(cardId);
        for (const field of ["issuer", "name", "rewardType", "sourceUrl", "applyUrl", "lastVerified"]) {
            if (!isNonEmptyString(card[field]))
                addIssue("must be a non-empty string", cardId, field);
        }
        ...
        for (const field of ["network", "bestFor", "tags", "exclusions"]) {
            if (!isStringArray(card[field]))
                addIssue("must be an array of non-empty strings", cardId, field);
        }
        ...
        for (const field of ["joiningFee", "annualFee", "forexMarkup"])
            validateMoney(card, cardId, field);
        validateMoney(card, cardId, "feeWaiverSpend", true);
        if (typeof card.popularityScore !== "number" ||
        ...
        if (!isValidUrl(card.sourceUrl))
            addIssue("must be a valid https URL", cardId, "sourceUrl");
        if (!isValidUrl(card.applyUrl))
            addIssue("must be a valid https URL", cardId, "applyUrl");
        ...
        if (card.sourceUrl !== undefined && isValidUrl(card.sourceUrl)) {
            const sourceUrl = new URL(card.sourceUrl);
            if (sourceUrl.search)
                addWarning("sourceUrl should be canonical without tracking query params", cardId, "sourceUrl");
        }
...
if (issues.length > 0) {
    for (const issue of issues)
        console.error(formatIssue("error", issue));
