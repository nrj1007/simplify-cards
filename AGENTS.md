...
- **No CSS framework** — one global stylesheet, `app/globals.css`, with a hand-rolled design
  system (see `app/AGENTS.md`). Icons via `lucide-react`. Image processing via `sharp`.
...
lib/            Server-side engine: data loading, scoring, Q&A, calc     (see lib/AGENTS.md)
...
- **`app/AGENTS.md`** — pages, components, the design system, and frontend conventions.
