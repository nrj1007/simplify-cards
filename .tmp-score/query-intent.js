...
function extractFocusedSpend(query) {
    const focusedCategory = spendCategoryAliases.find((entry) => entry.aliases.some((alias) => query.includes(`${alias} spend`) || query.includes(`${alias} spends`) || query.includes(`for ${alias}`)))?.category;
    if (!focusedCategory)
        return undefined;
    const spend = emptySpendProfile();
...
function parseQueryIntent(input) {
    ...
    return {
        ...
        wantsLifetimeFree: input.wantsLifetimeFree ??
            (normalizedQuery.includes("lifetime free") ||
                normalizedQuery.includes("life time free") ||
                normalizedQuery.includes("ltf")),
